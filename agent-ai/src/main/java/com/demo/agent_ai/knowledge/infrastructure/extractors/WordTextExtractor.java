...
@Component
public class WordTextExtractor extends AbstractFileProcessor {
    ...
    public boolean supports(UploadedFile file) {
        ...
        boolean hasWordContentType = contentType != null &&
                (contentType.contains("msword") || contentType.contains("wordprocessingml"));

        return hasWordContentType || filename.endsWith(".doc") || filename.endsWith(".docx");
    }
    ...
    protected String extract(UploadedFile file) {
        try (InputStream is = new ByteArrayInputStream(file.getContent());
            ...
            throw new RuntimeException("Cannot parse Word document", e);
        }
    }
}
